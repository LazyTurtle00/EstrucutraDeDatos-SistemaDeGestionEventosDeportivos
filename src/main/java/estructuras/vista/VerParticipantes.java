...
public class VerParticipantes extends JFrame {
    ...
    public VerParticipantes(boolean administrador){ this.administrador=administrador; setTitle("Ver Participantes"); setSize(900,540); setLayout(null); setLocationRelativeTo(null); setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); getContentPane().setBackground(new Color(220,220,220));
        JPanel barra=new JPanel(null); barra.setBounds(0,0,900,50); barra.setBackground(new Color(24,61,142)); add(barra); JLabel titulo=new JLabel("Consulta de Participantes"); titulo.setForeground(Color.WHITE); titulo.setFont(new Font("Arial",Font.BOLD,24)); titulo.setBounds(25,10,400,30); barra.add(titulo);
    ...
    private void cargarEventos(){ comboEventos.removeAllItems(); for(String i:ControladorSistema.obtenerEventosCombo()) comboEventos.addItem(i);} private void actualizar(){ area.setText(ControladorSistema.obtenerParticipantesTexto(ControladorSistema.obtenerIdDesdeCombo((String)comboEventos.getSelectedItem())));}
}
